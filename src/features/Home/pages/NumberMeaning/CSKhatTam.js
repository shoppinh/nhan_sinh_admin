...
const EditingNumberValueKTForm = React.lazy(() =>
  import("../../components/EditingNumberValueKTForm")
);
...
const CSKhatTam = () => {
  ...
  useEffect(() => {
    const fetchNumberByID = async () => {
      try {
        const res = await numberValueApi.getNumberKTByID(clickedEditingId);
